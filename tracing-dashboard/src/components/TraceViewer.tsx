...
export function TraceViewer({ endpoint, initialTraceId, highlightQuery = '' }: TraceViewerProps) {
  ...
  const filteredSpans = useMemo(() => {
    if (!selected) return [];
    if (!spanSearchQuery.trim()) return selected.spans;
    const q = spanSearchQuery.toLowerCase();
    return selected.spans.filter((s: any) =>
      (s.name || '').toLowerCase().includes(q) ||
      (s.kind || '').toLowerCase().includes(q)
    );
  }, [selected, spanSearchQuery]);

  const filteredTrace = useMemo(() => {
    if (!selected) return null;
    return { ...selected, spans: filteredSpans, span_count: filteredSpans.length };
  }, [selected, filteredSpans]);
          ...
          <>
            ...
            {viewMode === 'timeline' && selected && (
              <div className="flex-1 overflow-auto bg-white dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800 p-4">
                <TimelineView spans={filteredSpans} />
              </div>
            )}
            {viewMode === 'waterfall' && (
              <>
              ...
              <>
                <div className="flex-1 min-h-0">
                  <VirtualList
                    ...
                    renderItem={(s: any) => (
                      <button
                        onClick={() => setSelectedSpanId(selectedSpanId === s.id ? null : s.id)}
                        className={'w-full h-full text-left px-3 flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-100 dark:border-gray-800 ' +
                          (selectedSpanId === s.id ? 'bg-indigo-50 dark:bg-indigo-900/20' : '')}
                      >
